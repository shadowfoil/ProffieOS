// SF24 props file, includes 1,2 and 3 button modes.  Incorporates multi-blast,
...
//
// New #define SF24_NO_LOCKUP_HOLD
...
//
// #define SF24_STAB_ON
// #define SF24_SWING_ON
// #define SF24_TWIST_ON
// #define SF24_THRUST_ON
// #define SF24_TWIST_OFF
// #define SF24_FORCE_PUSH
//
// #define SF24_FORCE_PUSH_LENGTH 5
...
// #define SF24_LOCKUP_DELAY  (insert number here)
...
// Quote Player - Triple click while ON
//
//
// 2 Button:
// POWER
...
// AUX
// =============
// Blaster blocks - short click/double click
...
#ifndef PROPS_SABER_SF24_BUTTONS_H
#define PROPS_SABER_SF24_BUTTONS_H

#include "prop_base.h"
#include "../sound/hybrid_font.h"

#undef PROP_TYPE
#define PROP_TYPE SaberSF24Buttons

#ifndef MOTION_TIMEOUT
#define MOTION_TIMEOUT 60 * 15 * 1000
#endif

#ifndef SF24_SWING_ON_SPEED
#define SF24_SWING_ON_SPEED 250
#endif

#ifndef SF24_LOCKUP_DELAY
#define SF24_LOCKUP_DELAY 200
#endif

#ifndef SF24_FORCE_PUSH_LENGTH
#define SF24_FORCE_PUSH_LENGTH 5
#endif
...
#ifdef SF24_THRUST_ON
#define THRUST_GESTURE
#endif

#ifdef SF24_SEQ_QUOTE
#define SEQUENTIAL_QUOTE
#endif
...
// for the saber.
class SaberSF24Buttons : public PROP_INHERIT_PREFIX PropBase {
public:
  SaberSF24Buttons() : PropBase() {}
  const char* name() override { return "SaberSF24Buttons"; }
  ...
  void Loop() override {
    ...
    if (SaberBase::IsOn()) {
      ...
      if (auto_lockup_on_ &&
          ...
          millis() - clash_impact_millis_ > SF24_LOCKUP_DELAY &&
          SaberBase::Lockup()) {
      ...
      if (fabs(mss.x) < 3.0 &&
        ...
        if (millis() - push_begin_millis_ > SF24_FORCE_PUSH_LENGTH) {
      ...
      if (millis() - saber_off_time_ < MOTION_TIMEOUT) {
        ...
        if (!swinging_ && fusor.swing_speed() > SF24_SWING_ON_SPEED) {
          swinging_ = true;
...
// Revert colorchange witout saving (reset to Variation == 0)
    case EVENTID(BUTTON_POWER, EVENT_THIRD_SAVED_CLICK_SHORT, MODE_ON):
      ...
      STDOUT << "triple click recognized" << "\n";  
      if (SFX_quote) {        
        STDOUT << "Quote found" << "\n";
#ifdef SEQUENTIAL_QUOTE
          SFX_quote.SelectNext();
#endif
#ifndef SEQUENTIAL_QUOTE
          SFX_quote.Select(-1);
#endif
...
#else
  // 2 and 3 button
#ifndef SF24_NO_LOCKUP_HOLD
  case EVENTID(BUTTON_AUX, EVENT_SECOND_HELD, MODE_ON):
#else
  // in SF24_NO_LOCKUP_HOLD mode, multi-blaster is triggered by holding aux
  // while swinging
